...
const doorList = computed((): DoorItem[] =>
  ([
    { key: 'driver',    label: t('vehicle.doors_detail.driver'),    icon: 'door-open', open: props.driverDoorOpen },
    { key: 'passenger', label: t('vehicle.doors_detail.passenger'), icon: 'door-open', open: props.passengerDoorOpen },
    { key: 'rearLeft',  label: t('vehicle.doors_detail.rearLeft'),  icon: 'door-open', open: props.rearLeftDoorOpen },
    { key: 'rearRight', label: t('vehicle.doors_detail.rearRight'), icon: 'door-open', open: props.rearRightDoorOpen },
    { key: 'bonnet',    label: t('vehicle.doors_detail.bonnet'),    icon: 'car-burst', open: props.bonnetOpen },
...
const summary = computed((): string | null => {
  ...
  return effectiveLocked.value !== null
    ? (effectiveLocked.value ? t('vehicle.locked') : t('vehicle.unlocked'))
    : null
})
  ...
  >
    <div v-if="isLocked !== null" class="detail-list">
      <div class="detail-list__item detail-list__item--control">
        <font-awesome-icon :icon="effectiveLocked ? 'lock' : 'lock-open'" class="detail-list__item-icon" />
    ...
    <div v-if="doorList.length > 0" class="detail-list" :class="isLocked !== null ? 'mt-3' : ''">
      ...
      >
        ...
        <span class="detail-list__item-value">{{ door.open ? t('common.open') : t('common.closed') }}</span>
