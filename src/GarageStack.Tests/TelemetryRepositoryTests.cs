...
public class TelemetryRepositoryTests
{
    ...
    [Fact]
    public async Task GetGpsFixes_ReturnsPositionedRowsFromFromUpToTo_OldestFirst()
    {
        ...
        db.TelemetrySnapshots.AddRange(
            MakePoint(vehicle.Id, t0.AddMinutes(-1), 51.49, 0.0, 50),
            MakePoint(vehicle.Id, t0.AddMinutes(2), 51.52, 0.0, null),
            MakePoint(vehicle.Id, t0, 51.50, 0.0, 50),
            new TelemetrySnapshot { VehicleId = vehicle.Id, RecordedAt = t0.AddMinutes(1), BatteryVoltage = 12.6 },
            MakePoint(vehicle.Id, t0.AddMinutes(10), 51.60, 0.0, 50),
            MakePoint(vehicle.Id + 1, t0.AddMinutes(3), 51.53, 0.0, 50));
        await db.SaveChangesAsync(ct);

        var fixes = await new TelemetryRepository(db).GetGpsFixesAsync(vehicle.Id, t0, t0.AddMinutes(10), ct);

        Assert.Equal(
            [new TripPoint(t0, 51.50, 0.0, 50), new TripPoint(t0.AddMinutes(2), 51.52, 0.0, null)],
            fixes);
    }

    [Fact]
    public async Task GetFirstGpsFixAt_IsTheOldestRowCarryingAPosition()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var db = CreateDb();
        var vehicle = new Vehicle { Vin = "FAKEVN00000000001" };
        ...
        var t0 = new DateTime(2026, 1, 1, 10, 0, 0, DateTimeKind.Utc);
        var repo = new TelemetryRepository(db);
        db.TelemetrySnapshots.Add(new TelemetrySnapshot { VehicleId = vehicle.Id, RecordedAt = t0, BatteryVoltage = 12.6 });
        await db.SaveChangesAsync(ct);

        Assert.Null(await repo.GetFirstGpsFixAtAsync(vehicle.Id, ct));

        db.TelemetrySnapshots.AddRange(
            MakePoint(vehicle.Id, t0.AddHours(2), 51.50, 0.0, 0),
            MakePoint(vehicle.Id, t0.AddHours(1), 51.50, 0.0, 0));
        await db.SaveChangesAsync(ct);

        Assert.Equal(t0.AddHours(1), await repo.GetFirstGpsFixAtAsync(vehicle.Id, ct));
    }
